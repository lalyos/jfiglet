package com.github.lalyos.jfiglet;

import org.junit.Test;

import static org.junit.Assert.assertEquals;

/**
 * Created by Matthew on 17/05/2015.
 */
public class FigletFontTest {

    @Test
    public void testConvertMessage() throws Exception{
        String font = FigletFont.convertMessage(FigletFontTest.class.getClassLoader().getResourceAsStream("slant.flf"), "a");
        assertEquals("        \n" +
                "  ____ _\n" +
                " / __ `/\n" +
                "/ /_/ / \n" +
                "\\__,_/  \n" +
                "        \n",
                font.replaceAll("\r\n","\n").replaceAll("\r","\n"));

    }

    @Test
    public void testConvertMultiline() throws Exception{
        String font = FigletFont.convertMessage(FigletFontTest.class.getClassLoader().getResourceAsStream("slant.flf"), "ab\nc");
        assertEquals(   "          __  \n" +
                        "  ____ _ / /_ \n" +
                        " / __ `// __ \\\n" +
                        "/ /_/ // /_/ /\n" +
                        "\\__,_//_.___/ \n" +
                        "              \n" +
                        "              \n" +
                        "  _____       \n" +
                        " / ___/       \n" +
                        "/ /__         \n" +
                        "\\___/         \n" +
                        "              \n",
                font.replaceAll("\r\n","\n").replaceAll("\r","\n"));

    }

    @Test
    public void testConvertMultilineSmush() throws Exception{

        String font = new FigletFont(
                    FigletFontTest.class.getClassLoader().getResourceAsStream("slant.flf"))
                .overridehLayout(FittingRules.LAYOUT.FULL_WIDTH)
                .overridevLayout(FittingRules.LAYOUT.CONTROLLED_SMUSHING)
                .convert("ab\nc");
        assertEquals(   "            __  \n" +
                        "  ____ _   / /_ \n" +
                        " / __ `/  / __ \\\n" +
                        "/ /_/ /  / /_/ /\n" +
                        "\\__,_/  /_.___/ \n" +
                        "                \n" +
                        "  _____         \n" +
                        " / ___/         \n" +
                        "/ /__           \n" +
                        "\\___/           \n" +
                        "                \n",
                font.replaceAll("\r\n","\n").replaceAll("\r","\n"));

    }




}
